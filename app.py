...
class VoiceCloningApp:
    ...
    def _setup_audio_upload(self) -> None:
        ...
        if audio_file:
            ...
            try:
                ...
                if duration < 3:
                    st.warning(
                        "Audio is very short. For better results, use recordings of at least 5 seconds."
                    )
                elif duration > 30:
                    st.warning(
                        "Long audio may increase processing time. 10-20 seconds is ideal."
                    )
    ...
    def _setup_text_input(self) -> None:
        ...
        if st.button(
            "Generate Random Text", help="Get a randomly selected text template"
        ):
    ...
    def _setup_generation_controls(self) -> None:
        ...
        if generate_button:
            ...
            audio_loaded = (
                st.session_state.orig_wav is not None
                and isinstance(st.session_state.orig_wav, np.ndarray)
                and st.session_state.orig_wav.size > 0
            )
            text_provided = (
                st.session_state.text_input is not None
                and st.session_state.text_input.strip() != ""
            )

            if not audio_loaded or not text_provided:
                st.error(
                    "Please upload an audio file and enter the text to synthesize."
                )
            else:
    ...
    def _run_voice_cloning(self) -> None:
        ...
        def initialize_models():
            ...
            if st.session_state.model_instance is None:
                ...
                st.session_state.model_instance = Main(
                    original_encoder=(
                        st.session_state.encoder_choice == "Baseline Model (LSTM)"
                    )
                )
            else:
        ...
        try:
            ...
            st.success(
                "**Success:** Voice cloning process finished! The generated audio is available in the 'Output & Comparison' tab."
            )
    ...
    def _setup_output_tab(self) -> None:
        ...
        if (
            st.session_state.cloned_wav is None
            or not isinstance(st.session_state.cloned_wav, np.ndarray)
            or st.session_state.cloned_wav.size == 0
        ):
            st.info(
                "Generate a cloned voice first in the 'Source Audio and Cloning' tab."
            )
            return
    ...
    def _setup_cloned_voice_section(self) -> None:
        ...
        with col1:
            ...
            if (
                "cloned_wav" in st.session_state
                and isinstance(st.session_state.cloned_wav, np.ndarray)
                and st.session_state.cloned_wav.size > 0
    ...
    def _setup_voice_characteristics_profile(self) -> None:
        ...
        try:
            ...
            if (
                hasattr(st.session_state, "cloned_wav")
                and st.session_state.cloned_wav is not None
            ):
                ...
                with st.spinner("Analyzing voice characteristics..."):
                    voice_metrics = calculate_voice_metrics(
                        st.session_state.cloned_wav, st.session_state.sr
                    )

                    if voice_metrics is None:
                        st.warning(
                            "Could not analyze voice characteristics from the audio data."
                        )
                        return
                ...
                with col1:
                    with st.expander(
                        f"Timbre Richness: {voice_metrics['Timbre Richness']:.1f}/100",
                        expanded=True,
                    ):
                        st.markdown(
                            """
                        ...
                        if score >= 90:
                            st.success(
                                "Excellent: This voice has a rich, full-bodied timbre with well-balanced harmonics."
                            )
                        elif score >= 75:
                            st.info(
                                "Good: This voice has a pleasant timbre with a decent harmonic structure."
                            )
                        else:
                            st.warning(
                                "Moderate: The voice timbre could benefit from more harmonic richness."
                            )

                with col2:
                    with st.expander(
                        f"Pitch Stability: {voice_metrics['Pitch Stability']:.1f}/100",
                        expanded=True,
                    ):
                        st.markdown(
                            """
                        ...
                        if score >= 90:
                            st.success(
                                "Excellent: The voice shows very stable pitch with natural micro-variations."
                            )
                        elif score >= 75:
                            st.info(
                                "Good: The voice has good pitch stability with appropriate variations."
                            )
                        else:
                            st.warning(
                                "Moderate: The pitch control could be more consistent."
                            )
                ...
                with col3:
                    with st.expander(
                        f"Articulation: {voice_metrics['Articulation']:.1f}/100",
                        expanded=True,
                    ):
                        st.markdown(
                            """
                        ...
                        if score >= 90:
                            st.success(
                                "Excellent: The speech is very clearly articulated with distinct consonants and vowels."
                            )
                        elif score >= 75:
                            st.info(
                                "Good: The speech has good clarity with well-defined sounds."
                            )
                        else:
                            st.warning(
                                "Moderate: Some phonemes could be more clearly articulated."
                            )

                with col4:
                    with st.expander(
                        f"Speech Rhythm: {voice_metrics['Speech Rhythm']:.1f}/100",
                        expanded=True,
                    ):
                        st.markdown(
                            """
                        ...
                        if score >= 90:
                            st.success(
                                "Excellent: The speech has very natural rhythm with appropriate pauses and pacing."
                            )
                        elif score >= 75:
                            st.info(
                                "Good: The speech has a natural flow with good pacing."
                            )
                        else:
                            st.warning(
                                "Moderate: The rhythm could be more natural with better pause patterns."
                            )
            else:
                st.warning(
                    "Voice characteristics profile requires generating a cloned voice first."
                )
